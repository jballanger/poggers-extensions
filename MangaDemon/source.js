...
class MangaDemon {
        ...
        this.homePageSections = [
            {
                ...
                parser: MangaDemonParser_1.parseMostViewedTodayPage,
            },
            {
                section: App.createHomeSection({
                    id: 'latestUpdates',
                ...
                parser: MangaDemonParser_1.parseLatestUpdatePage,
            },
            {
                section: App.createHomeSection({
                    id: 'newManga',
                ...
                parser: MangaDemonParser_1.parseNewMangaPage,
            },
        ];
    }
    ...
    async getHomePageSections(sectionCallback) {
        ...
        this.homePageSections.forEach((pageSection) => {
            ...
            const requestPromise = this.requestManager
                ...
                .then(async (response) => {
                const pageRoot = (0, cheerio_1.load)(response.data);
    ...
    async getViewMoreItems(homePageSectionId, metadata) {
        const results = [];
        const section = this.homePageSections.find((homePageSection) => homePageSection.section.id === homePageSectionId);
        if (section) {
    ...
    async getMangaDetails(mangaId) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const pageRoot = (0, cheerio_1.load)(response.data);
        const mangaInfo = await (0, MangaDemonParser_1.parseMangaPage)(pageRoot);
    ...
    async getChapters(mangaId) {
        ...
        const pageRoot = (0, cheerio_1.load)(result.data);
        const chapters = await (0, MangaDemonParser_1.parseMangaChapters)(pageRoot);
        return chapters;
    }
    async getChapterDetails(mangaId, chapterId) {
        ...
        const result = await this.requestManager.schedule(request, 1);
        const pageRoot = (0, cheerio_1.load)(result.data);
        const chapterPages = await (0, MangaDemonParser_1.parseMangaChapterDetails)(pageRoot);
    ...
    async searchManga(search) {
        ...
        const result = await this.requestManager.schedule(request, 1);
        const pageRoot = (0, cheerio_1.load)(result.data);
        const searchResult = await (0, MangaDemonParser_1.parseMangaSearch)(pageRoot);
        return App.createPagedResults({
            results: searchResult,
        });
    }
    async browseManga(query, metadata) {
        const genres = [];
        ...
        const result = await this.requestManager.schedule(request, 1);
        const pageRoot = (0, cheerio_1.load)(result.data);
        const searchResult = await (0, MangaDemonParser_1.parseMangaBrowse)(pageRoot);
        return App.createPagedResults({
            results: searchResult,
            metadata: {
                page: page + 1,
    ...
    async getSearchTags() {
        ...
        const result = await this.requestManager.schedule(request, 1);
        const pageRoot = (0, cheerio_1.load)(result.data);
        const tags = await (0, MangaDemonParser_1.parseTagsList)(pageRoot);
...
},{"./MangaDemonParser":140,"@paperback/types":61,"cheerio":74}],140:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.parseTagsList = exports.parseMangaBrowse = exports.parseMangaSearch = exports.parseMangaChapterDetails = exports.parseMangaChapters = exports.parseMangaPage = exports.parseNewMangaPage = exports.parseLatestUpdatePage = exports.parseMostViewedTodayPage = void 0;
...
const mangaIdToImageId = (mangaId) => {
...
const parseMostViewedTodayPage = ($) => {
...
exports.parseMostViewedTodayPage = parseMostViewedTodayPage;
const parseLatestUpdatePage = ($) => {
...
exports.parseLatestUpdatePage = parseLatestUpdatePage;
const parseNewMangaPage = ($) => {
...
exports.parseNewMangaPage = parseNewMangaPage;
const parseMangaPage = ($) => {
    ...
    const tags = tagsList.map((element) => {
        ...
        const genreParam = tagLinkHref.split('&').find((param) => param.includes('genre[]')) || '';
...
exports.parseMangaPage = parseMangaPage;
const parseMangaChapters = ($) => {
    ...
    const chapters = chaptersListItems.map((element) => {
        ...
        const chapterUpdate = $(element).children().last().children().last().attr('date') || '';
...
exports.parseMangaChapters = parseMangaChapters;
const parseMangaChapterDetails = ($) => {
    const chapterImages = $('.imgholder').toArray();
    const chapterPages = chapterImages.map((image) => encodeURI($(image).attr('src') || ''));
    return chapterPages;
};
exports.parseMangaChapterDetails = parseMangaChapterDetails;
const parseMangaSearch = ($) => {
...
exports.parseMangaSearch = parseMangaSearch;
const parseMangaBrowse = ($) => {
    ...
    const tiles = searchResults.map((searchResult) => {
        ...
        const image = encodeURI($('.novel-cover img', searchResult).attr('src') || '');
        return App.createPartialSourceManga({
            mangaId,
            image,
            title: mangaTitle,
...
exports.parseMangaBrowse = parseMangaBrowse;
const parseTagsList = ($) => {
